...
| T6 | Box geometry derived from measured text plus theme padding; vertical centring; same-rank size normalisation; **elastic fit** — one uniform type-scale factor chosen within the theme's band; `FitError` below the minimum legible size | `drawspec.geometry` | M | T8 | `node_overlap_count == 0` | `0` — across 5 graph shapes × 2 directions and 100 pseudo-random graphs; deterministic, and stable under a reordered input | `uv run pytest tests/test_layout.py -q` (139 passed) | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
