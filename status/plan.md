...
| T6 | Box geometry derived from measured text plus theme padding; vertical centring; same-rank size normalisation; **elastic fit** — one uniform type-scale factor chosen within the theme's band; `FitError` below the minimum legible size | `drawspec.geometry` | M | T14 | `unlabelled_axis_count == 0` | `0` — impossible by construction: the schema makes `label` required on both axes, so it is refused at parse time with a JSON pointer | `uv run pytest tests/test_kinds_chart.py -q` (23 passed); `make gallery` | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
