...
|----|------|----------|---------|-----|-----|------|
| T18 | `nondeterministic_reruns == 0` | `0` — 9 documents × 2 profiles, byte-identical in this interpreter and in two fresh ones seeded `PYTHONHASHSEED=1` and `7919`; renders with `PATH=""`, so `system_dependencies == 0` is measured rather than intended; the brief's three worst fixtures (020, 067, 047) all render | `uv run pytest tests/test_acceptance.py -q` (17 passed); `uv run pytest -q` (868 passed, 1 skipped) | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
| T17 | `embedding_safety_violations == 0`, **per profile** | `0` and `0` — 9 reference documents × 2 profiles, each measured on its own; two diagrams on one HTML page share no id and offer no selector; the greyscale signature count is 7 for 6 node roles, so no pair collapses in black and white. The PDF conversion was really run (`chromium --print-to-pdf` on the standalone flow → 32 kB `%PDF-1.4`, no `currentColor` in the bytes); the suite asserts the converter's preconditions and **skips** the conversion itself where no converter is installed | `uv run pytest tests/test_embedding.py -q` (41 passed, 1 skipped) | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
| T16 | `unrepresentable_failure_families == 11` | `11` — one test per family in `docs/brief.md`, each naming which of the two answers it rests on (the schema has no field for it, or an invariant excludes it); the count test asserts every family in `FAMILIES` has a test, so none can be dropped to make the number work | `uv run pytest tests/test_failure_families.py -q` (21 passed) | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
| T15 | `cli_exit_code_mismatches == 0` | `0` — every code in §5.4 asserted as a value: `render` 0/1/2, `validate` 0/1 with the JSON pointer of every violation, `theme check` 0/1, `schema` 0, and a missing command 2 | `uv run pytest tests/test_cli.py -q` (22 passed) | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
| T11 | `text_overflow_count == 0` | `0` — every text run in the finished flow scene is inside the box that was measured for it or is a placed label; the reference flow and tree both render to safe SVG in both profiles, and the gallery now draws all 9 reference documents with none skipped | `uv run pytest tests/test_kinds_graph.py -q` (15 passed); `make gallery` | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
...
| T14 | `unlabelled_axis_count == 0` | `0` — impossible by construction: the schema makes `label` required on both axes, so it is refused at parse time with a JSON pointer | `uv run pytest tests/test_kinds_chart.py -q` (23 passed); `make gallery` | `claude/continuation-yq88jv` | CPython 3.12, Linux | 2026-08-12 |
