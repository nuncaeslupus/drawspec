...
|----|-------------|---------|------|---------|------|-------|
| T1 | `Scene` primitives (`Rect`, `Ellipse`, `Polygon`, `Path`, `TextRun`), the SVG emitter, and the inline-safety validator it enforces on its own output | `drawspec.scene`, `drawspec.emit` | M | — | `inline_safety_violations == 0` | `test_emit_any_scene_produces_no_style_element` in `tests/test_emit.py` — emitted SVG contains no `<style>` element. `test_emit_two_documents_on_one_page_produces_no_duplicate_ids` — concatenating two renders yields no repeated `id`. `test_emit_default_theme_strokes_use_currentcolor` — no literal colour outside theme-declared values. `test_emit_scene_with_undeclared_role_raises_error` — a primitive tagged with a role the theme does not declare raises. `test_emit_same_scene_twice_produces_identical_bytes` — two emissions are byte-equal |
| T2 | `Theme` frozen dataclass, TOML loader, and the greyscale invariant checked at load; one bundled default theme | `drawspec.theme` | M | — | `greyscale_ambiguous_role_pairs == 0` | `test_theme_roles_differing_only_by_colour_raises_themeerror` in `tests/test_theme.py` — two roles with identical shape/dash/weight and a luminance delta below 0.25 are rejected. `test_theme_roles_differing_by_dash_pattern_loads_successfully` — a non-colour channel difference is accepted. `test_bundled_default_theme_loads_without_violations` — the shipped theme passes its own invariant. `test_theme_missing_required_role_raises_themeerror` — a theme omitting a vocabulary role is rejected |
...
| T4 | Document model, validation and JSON Schema export; `additionalProperties: false` throughout so forbidden fields fail by name | `drawspec.schema` | L | — | `forbidden_field_acceptance_count == 0` | `test_document_with_forbidden_field_raises_documenterror_naming_field` in `tests/test_schema.py` — parametrized over every field in spec §5.1's rejection table; each raises and the message names the field. `test_document_missing_version_raises_documenterror` — `version` is required. `test_document_with_role_outside_vocabulary_raises_documenterror` — an undeclared role is rejected. `test_exported_schema_validates_every_bundled_example` — the emitted JSON Schema accepts all example documents |
...
| T6 | Box geometry derived from measured text plus theme padding; vertical centring; same-rank size normalisation; `FitError` below the minimum legible size | `drawspec.geometry` | M | T5 | `text_overflow_count == 0` | `test_box_contains_its_text_with_full_padding_on_all_four_sides` in `tests/test_box.py` — the text extents sit inside the box inset by the theme padding, bottom included. `test_box_text_is_vertically_centred_within_half_a_pixel` — centring is computed from ascent/descent, not approximated. `test_box_content_below_min_legible_size_raises_fiterror` — refuses to shrink type. `test_boxes_of_the_same_rank_are_normalised_to_equal_size` — peers match |
...
| T17 | Acceptance close-out: the three reference documents rendered and committed, a README gallery, and the determinism and zero-dependency checks | `drawspec`, `README.md` | M | T15, T16 | `nondeterministic_reruns == 0` | `test_every_fixture_renders_identically_on_a_second_run` in `tests/test_acceptance.py` — byte-identical reruns across the whole fixture set. `test_package_renders_with_no_system_binaries_available` — import and render succeed with `PATH` emptied of non-Python executables. `test_three_reference_documents_render_without_fit_or_layout_errors` — the brief's second acceptance test passes |
...
T11 completes the graph branch. T15 and T16 gather, T17 closes.
...
The critical path is T3 → T5 → T6 → T7 → T8 → T9 → T10 → T11 → T17. Text
measurement gates everything, which is why it is Large and first.
